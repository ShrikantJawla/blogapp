...
import { resetLoginStatus } from "../../redux/auth/auth.actions";
...
const Login = (props: Props) => {
	...
	const { loading, error, token, loginStatus } = useSelector(
		(s: StoreType) => s.auth
	);

	useEffect(() => {
		if (!error && token && loginStatus.token) {
			dispatch(resetLoginStatus());
			router.back();
			toast({
				title: `${
					loginStatus.status === 1 ? "Login successful" : "Login failed"
				}`,
				description: loginStatus.message,
				position: "top",
				isClosable: true,
				status: `${loginStatus.status === 1 ? "success" : "warning"}`,
				duration: 4000,
			});
		}
	}, [error, token]);
